...
export type MwMessages = (
  key: string,
  ...params: readonly (string | HTMLElement)[]
) => MwMessage;
...
export type MwApiParameter =
  | MwApiParameterPrimitive
  | readonly MwApiParameterPrimitive[];
export type MwApiParameters = Record<string, MwApiParameter>;
export type MwApiParametersWithout<K extends string> = MwApiParameters & {
  [k in K]?: never;
};
...
export interface MwApi {
  ...
  assertCurrentUser(
    parameters: MwApiParametersWithout<'assert' | 'assertuser'>,
  ): MwApiParameters;
}

/** @see: https://doc.wikimedia.org/mediawiki-core/master/js/#!/api/mw.ForeignApi */
export type MwForeignApiConstructor = new (
  url: string,
  options?: ApiOptions,
) => MwApi;
