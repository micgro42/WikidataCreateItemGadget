...
export default class ApiCore implements Api {
  ...
  public postWithEditTokenAndAssertUser<action extends ApiAction>(
    ...
    return Promise.resolve(
      // turn jQuery promise into native one
      this.api
        .postWithEditToken(
  ...
  public postWithEditToken<action extends ApiAction>(
    ...
    return Promise.resolve(
      // turn jQuery promise into native one
      this.api
        .postWithEditToken(this.mapParameters(params))
        .catch(this.mwApiRejectionToError),
    );
  }

  public get<action extends ApiAction>(
    params: ApiParams<action>,
  ): Promise<ApiResponsesMap[action]> {
    return Promise.resolve(
      // turn jQuery promise into native one
      this.api
        .get(this.mapParameters(params))
        .catch(this.mwApiRejectionToError),
    );
  }
      ...
      default: {
        ...
        if (result.error) {
          ...
          throw new TechnicalProblem(
            'mw.Api rejected but result does not contain error(s)',
          );
        }
      }
    }
